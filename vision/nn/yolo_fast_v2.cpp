...
void YoloFastV2::load_data_file(const std::string &file_path) {
  ...
  for (std::string line; std::getline(ifs, line);) {
    ...
    temp_key_value[1] = *(temp_key_value[1].cend() - 1) == '\r' ? temp_key_value[1].substr(0, temp_key_value[1].length() - 1) : temp_key_value[1];
...
std::map<std::string, std::vector<YoloFastV2Result>>
YoloFastV2::process(const cv::Mat &img, const cv::Mat &detect_result) {
