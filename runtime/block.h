...
class ChainBlock : public Block<typename std::remove_reference_t<Block1_>::In, typename std::remove_reference_t<Block2_>::Out> {
  using Block1 = typename std::remove_reference_t<Block1_>;
  using Block2 = typename std::remove_reference_t<Block2_>;
  static_assert(std::is_same_v<typename Block1::Out, typename Block2::In>, "Unmatched input and output types for two blocks.");
public:
